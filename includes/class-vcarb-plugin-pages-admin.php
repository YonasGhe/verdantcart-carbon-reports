...
final class VCARB_Plugin_Pages_Admin
{
    ...
    public static function is_plugin_page(int $page_id): bool
    {
        ...
        return
            (string) get_post_meta($page_id, self::META_PLUGIN_PAGE, true) === '1'
            || (string) get_post_meta($page_id, self::META_MANAGED_PAGE, true) === '1';
    }
    ...
    private static function get_current_filter_value(): string
    {
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Read-only admin filter; unslashed immediately.
        ...
        return in_array($value, ['plugin', 'regular'], true)
            ? $value
            : '';
    }
    ...
    private static function get_admin_post_type(WP_Query $query): string
    {
        ...
        if (isset($_GET['post_type'])) {
            ...
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Read-only admin request value; unslashed immediately.
